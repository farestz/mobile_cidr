...
Использование в xray-конфиге (DirectIp / ProxyIp / BlockIp):
    ...
    "ext:/path/to/mobile-ru.dat:!mobile"     # всё, кроме мобильных (инверсия)
    "ext:/path/to/mobile-ru.dat:mts"

ВАЖНО про инверсию: xray-core НЕ читает поле `reverse_match` из
protobuf — инверсия делается только `!`-префиксом на уровне правила
(см. xray-core/common/geodata/rule_parser.go). Поэтому НЕ генерируем
"_NOT"-теги — они были бы просто дубликатом основного списка CIDR и
сбивали бы с толку.

Если файл подменяет основной geoip.dat (Geoipurl в happ-конфиге):
    "geoip:mobile", "!geoip:mobile", "geoip:mts"
